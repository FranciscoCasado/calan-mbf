...
bot = telegram.Bot('468799235:AAFTHYgMC1JqlSaYW1ZgpgZHMTyZ58Cc2H')
chat_id = 17906513

ip_addr = '192.168.1.13'
fpga = None  # katcp_wrapper.FpgaClient(ip_addr, 7147, timeout=10, logger=logger)
time.sleep(1)
...
for i in range(len(theta)):
    ...
    for j in range(len(phi)):
        ...
        # pow = np.array([[0.5]*256, [0.4]*256])
        # data = np.exp(-((i-len(theta)/2.0)/(len(theta)))**2 - ((j-len(phi)/2.0)/(len(theta)))**2)
        data = i
        data_dB = 10*np.log10(data)
        value[len(theta)-i-1][j] = data
